...
import { getAllPricePeriods, deletePricePeriod, duplicatePricePeriod } from '@/integrations/supabase/services/periodService';
...
const PeriodsPage: React.FC = () => {
  ...
  return <div className="space-y-6">
      ...
      <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="prices" className="space-y-6 mt-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {(['Standard', 'Luxo', 'Super Luxo', 'Master'] as CategoryType[]).map(category => <Button key={category} variant="outline" className="h-auto flex-col py-6 border-2" onClick={() => handleOpenCategoryPrice(category)}>
      ...
      <PeriodDialog isOpen={isPeriodDialogOpen} onOpenChange={setIsPeriodDialogOpen} onSuccess={handlePeriodSuccess} periodId={editingPeriodId || undefined} />
      
      <CategoryPriceDialog category={currentCategory as CategoryType} isOpen={isCategoryPriceDialogOpen} onOpenChange={setIsCategoryPriceDialogOpen} onUpdate={handleCategoryPriceUpdate} />
      
      {periodToDuplicate && <DuplicateDialog isOpen={isDuplicateDialogOpen} onOpenChange={setIsDuplicateDialogOpen} title="Duplicar Período" description={`Duplicar o período "${periodToDuplicate.name}" com todos os seus preços associados.`} nameLabel="Nome do Novo Período" originalName={periodToDuplicate.name} onConfirm={handleDuplicatePeriod} />}
      ...
      <AlertDialog open={isPermanentDeleteDialogOpen} onOpenChange={setIsPermanentDeleteDialogOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={loading}>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={confirmPermanentDelete} disabled={loading} className="bg-red-600 hover:bg-red-700">
